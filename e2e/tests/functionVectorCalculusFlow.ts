...
export async function functionVectorCalculusFlow(page: Page, info: TestInfo, app?: ElectronApplication): Promise<void> {
  ...
  // Merely viewing structured input preserves the original spelling and the ordered Cartesian variables.
  await expect(math.locator('textarea')).toHaveValue(source);
  ...
  await captureManualDetail(page, info, { name: 'function-vector-calculus', dialog, script: new URL(import.meta.url),
    fixture: { source, expected: 'Y=3*X^2', bounds: { X: [-1, 1], Y: [-7, 7], Z: [-1, 1] } } });
  await page.setViewportSize({ width: 1440, height: 900 });
  const created = await beginRecompute(page); await apply.click(); await waitForRecompute(page, created);
  const original = await savePart(page, info, 'function-vector-calculus.pcad', app);
  const curve = original.sketches.flatMap(sketch => sketch.features).find(feature => feature.kind === 'functionCurve');
  if (curve?.kind !== 'functionCurve') throw new Error('勾配から作った曲線が保存されていません。');
  expect(curve.definition.formula).toMatchObject({ outputs: { Y: { source, angleUnit: 'degree' } } });
  await reopenPart(page, info, 'function-vector-calculus.pcad', app);
  ...
  const edited = await savePart(page, info, 'function-vector-calculus-edited.pcad', app);
  ...
  expect((await savePart(page, info, 'function-vector-calculus-undone.pcad', app)).sketches).toEqual(original.sketches);
  ...
  const saved = await savePart(page, info, 'function-hessian-surface.pcad', app);
  ...
  expect(surface).toMatchObject({ definition: { formula: { kind: 'coordinate-surface', output: 'Z',
    expression: { source: 'component(hessian(X^2*Y,[X,Y]),1,2)' } } } });
  await reopenPart(page, info, 'function-hessian-surface.pcad', app);
