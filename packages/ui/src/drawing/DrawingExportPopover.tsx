...
export function DrawingExportPopover({ onClose }: { readonly onClose: () => void }): React.JSX.Element {
  ...
  return <form className="pcad-drawing-input" data-help-topic="drawing-export" aria-label={t('drawing.export.title')} onSubmit={(event) => {
    ...
    {image ? <label>{t('drawing.export.dpi')}<select aria-label={t('drawing.export.dpi')} title={t('drawing.export.dpiHint')} value={dpi} disabled={busy} onChange={(event) => {
    ...
    <div><button type="submit" className="pcad-button" title={t('drawing.export.saveHint')} disabled={busy}>{t('drawing.export.save')}</button>
      <button type="button" className="pcad-button" title={t('drawing.export.closeHint')} disabled={busy} onClick={onClose}>{t('drawing.action.close')}</button></div>
  </form>;
}
