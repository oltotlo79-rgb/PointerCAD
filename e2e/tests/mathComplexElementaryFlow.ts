...
/** Principal complex functions, explicit real components, original poles and saved edits. */
export async function mathComplexElementaryFlow(page: Page, info: TestInfo, app?: ElectronApplication): Promise<void> {
  ...
  await name.fill('複素関数の値'); await name.press('Enter');
  ...
  for (const [source, reason] of [
    ['component([7,atan(i)],1)', '指定した値では逆関数が有限になりません。'],
    ['0*ln(i-i)', '対数の真数が0になる式は使えません。'],
    ['1/(i-i)', '0では割れません。'],
  ] as const) {
    await input.fill(source); await expect(result).toHaveText(reason, { timeout: 15_000 });
    await expect(apply).toBeDisabled();
  }
  await input.fill('sqrt(-4)');
  await expect(result).toContainText('複素数'); await expect(apply).toBeDisabled();
  for (const [formula, value] of [['re(exp(i*pi))','= -1'], ['im(sqrt(3+4*i))','= 1'],
    ['re(asin(2))','= 90'], ['im(ln(-1))','3.141592653'], ['im(sin(complex(0,180/pi)))','1.175201193']] as const) {
    await input.fill(formula); await expect(result).toContainText(value); await expect(apply).toBeEnabled();
  }
  const source = 're(exp(i*pi))+im(sqrt(3+4*i))+re(asin(2))', changed = 'im(ln(-1))', initialValue = 90, changedValue = Math.PI;
  await input.fill(source); await expect(result).toContainText('= 90');
  ...
  await expect(input).toHaveValue(source); await expect(result).toContainText('= 90');
  await page.setViewportSize({ width: 1440, height: 1100 });
  await captureManualDetail(page, info, { name: 'math-complex-elementary', dialog, script: new URL(import.meta.url),
  ...
  const popover = page.locator('.pcad-popover'), coordinate = 'coef("複素関数の値")';
  await popover.getByRole('button', { name: /数式で入力/u }).first().click();
  await waitForMathEditorText(dialog); await input.fill(coordinate); await expect(result).toContainText('= 90');
  await apply.click(); await expect(dialog).toHaveCount(0);
  await popover.getByRole('button', { name: '決定', exact: true }).click();
  await popover.getByRole('button', { name: '取消', exact: true }).click();
  const saved = await savePart(page, info, 'complex-elementary.pcad', app);
  const parameter = saved.parameters.find(parameter => parameter.name === '複素関数の値');
  ...
  await reopenPart(page, info, 'complex-elementary.pcad', app);
  await page.getByRole('tab', { name: 'パラメータ', exact: true }).click();
  await row.getByRole('button', { name: '数式で入力', exact: true }).click();
  await waitForMathEditorText(dialog); await expect(input).toHaveValue(source); await expect(result).toContainText('= 90');
  await page.keyboard.press('F1');
  await expect(page.locator('.pcad-help__article')).toContainText('複素数の関数と主値');
  await page.keyboard.press('Escape');
  await input.fill(changed); await expect(result).toContainText('3.141592653'); await apply.click(); await expect(dialog).toHaveCount(0);
  const edited = await savePart(page, info, 'complex-elementary-edited.pcad', app);
  ...
  expect(edited.parameters.find(parameter => parameter.name === '複素関数の値')?.value.source).toBe(changed);
  await undoMathEdit(page);
  const undone = await savePart(page, info, 'complex-elementary-undone.pcad', app);
  ...
  expect(undone.parameters.find(parameter => parameter.name === '複素関数の値')?.value.source).toBe(source);
}
