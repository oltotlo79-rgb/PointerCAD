...
export async function mathExactLinearFlow(page: Page, info: TestInfo, app?: ElectronApplication): Promise<void> {
  ...
  await name.fill('行列計算の成分'); await name.press('Enter');
  ...
  const samples: readonly (readonly [string, number])[] = [
    ...
    ['component(qrq([[sqrt(2),0],[0,1]]),1,1)', 1],
    ['component(qrr([[sqrt(2),1],[0,1]]),1,1)', Math.SQRT2],
    ['im(component(qrq([[i,0],[0,1]]),1,1))', 1],
    ['component(lup([[0,sqrt(2)],[1,1]]),1,2)', 1],
    ['component(lul([[sqrt(2),1],[2,0]]),2,1)', Math.SQRT2],
    ['component(luu([[sqrt(2),1],[2,0]]),2,2)', -Math.SQRT2],
    ['component(characteristiccoefficients([[sqrt(2),1],[1,sqrt(3)]]),3)', Math.sqrt(6)-1],
    ['component(eigenspace([[sqrt(2),1],[0,sqrt(2)]],sqrt(2)),1,1)', 1],
    ['component(singularvalues([[2,1,0],[1,2,1],[0,1,2]]),1)', 2+Math.SQRT2],
    ['component(svds([[sqrt(2),0],[0,1e-40]]),2,2)/1e-40', 1],
    ['im(component(svdu([[i,0],[0,sqrt(2)]]),1,2))', 1],
    ['component(svdv([[i,0],[0,sqrt(2)]]),2,1)', 1],
    ['component(eigenvalues([[sqrt(2),1],[0,sqrt(3)]]),1)', Math.SQRT2],
  ...
  const invalid = [
    ...
    'component(qrq([[sqrt(2),1],[2]]),1,1)',
    'component(eigenvalues([[sqrt(2),1]]),1)',
    'component(singularvalues([[sqrt(2),1]]),2)',
    'component(characteristiccoefficients([[sqrt(2),1]]),1)',
    'component(eigenspace([[sqrt(2),0],[0,1]],0),1,1)',
  ...
  const source = 'component(singularvalues([[2,1,0],[1,2,1],[0,1,2]]),2)';
  ...
  expect(saved.parameters.find(parameter => parameter.name === '行列計算の成分')?.value).toMatchObject({
