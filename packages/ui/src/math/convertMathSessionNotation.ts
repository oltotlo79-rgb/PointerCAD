...
export async function convertMathSessionNotation(client: MathWorkerClient, input: MathEditorInput,
  ...
  if (!sameMathIdentity(input.identity, request.identity) || input.source !== request.source
    || input.notation !== request.notation || input.angleUnit !== request.angleUnit
    || !sameMathDeclarations(input.declarations, request.declarations ?? request.definition?.declarations)) throw new Error('The math input changed before conversion');
  ...
  if (!sameMathIdentity(completion.identity, input.identity) || converted === undefined || converted === null
    || converted.inputNotation !== target || converted.angleUnit !== input.angleUnit
    || !sameMathDeclarations(input.declarations, converted.declarations)) throw new Error('Math notation conversion did not return the requested input');
