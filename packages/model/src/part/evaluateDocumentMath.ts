...
import { bindMathCompositionNames, collectMathCoefficients, evaluateExpression, evaluateExpressionExact, mathScalarExpression,
  originalCoefficientExpression, exactCoefficientScalar,
  type ExpressionValue, type MathCompositionCoefficient } from '@pointercad/expression';
...
export async function evaluateDocumentMath(document: PartDocument, context: DocumentMathContext): Promise<DocumentMathResult> {
  ...
  for (const parameter of document.parameters) {
    if (byName.has(parameter.name) || (parameter.mathId !== undefined && (!isParameterMathId(parameter.mathId) || byId.has(parameter.mathId)))) {
  ...
  const evaluate = async (value: ExpressionValue): Promise<ValueResult> => {
    ...
    if (definition === undefined) {
      ...
      if (!exact.ok) return { ok: false, message: exact.error.message };
    ...
    for (const reference of references) {
      ...
      if (parameter === undefined || parameter.name !== reference.label) return { ok: false, message: `係数の参照先を確認できません: ${reference.label}` };
      if (coefficient === undefined) return { ok: false, message: `参照する係数を計算できません: ${reference.label}` };
  ...
  try {
    ...
    for (const name of order) {
      ...
      const result = await evaluate(parameter.value);
      if (!current()) return cancelled();
      ...
      try {
        coefficient = { id: parameter.mathId ?? `legacy-coefficient:${document.parameters.indexOf(parameter)}`, label: name,
    ...
    rememberMathParameterEvaluation(parameters, analysis);
    ...
    if (failures.length > 0) return { ok: false, cancelled: false, failures,
      recompute: { document: evaluated, analysis, invalidInputs } };
