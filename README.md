...
### 数式で決まる輪郭や曲面を、設計する形として扱える

曲線や曲面を数式で定義し、**X・Y・Zそれぞれに指定した範囲だけ**を形にできます。座標のX/Y/Z、曲線や曲面の変数T/U/V、名前付きの係数を別々に選ぶため、数学の記号と座標を区別して入力できます。閉じた面は立体として扱い、描画範囲で切れた開いた面とは区別します。

作った関数の上に点を置く際は、分かっている座標を1つまたは2つ指定して候補を探し、図と座標の一覧から選べます。元の式や係数を変更した後も、保存した式を再編集しながら形や点の位置を調整できます。数式から得た形を眺めるところから、設計に使う点や断面を決めるところまでつながります。[関数曲線](packages/help-content/docs/ja/function-curve.md) · [関数曲面](packages/help-content/docs/ja/function-surface.md) · [関数上の点](packages/help-content/docs/ja/function-point.md)
...
比較は各製品の公式説明とPointerCAD開発版の機能に基づく、操作方法の整理です（表内3件の公式資料を2026年9月24日に再確認し、記載内容と相違なし）。
...
行列を分解した結果も、必要な成分を選んだ式として位置や寸法に使えるよう実装・検証を進めています。左の基底・特異値・右の基底を区別し、選択した行と列を保存するため、元の行列を直した後も指定を追えます。[特異値分解と成分の利用](packages/help-content/docs/ja/math-input.md#特異値分解の3つの行列を使う)

テンソルの成分や整数の条件から位置を決める入力も実装・検証中です。**配列のどの成分をXYZに使うか、条件が成り立つ場合にどの数値を使うかを明示し、その選択ごと式として保存する**ことを目指しています。テンソル積と成分ごとの積、整数の商と余り、素数・約数・合同の条件を区別し、未確定の結果を座標へ黙って取り込みません。固有値を指定して求めた基底も、成分と複素数の実部・虚部を明示して利用する実装を進めています。[テンソルと座標](packages/help-content/docs/ja/math-input.md#テンソルの軸と成分を指定する) · [整数の条件と座標](packages/help-content/docs/ja/math-input.md#整数の商約数素数合同) · [固有空間と成分](packages/help-content/docs/ja/math-input.md#固有値を指定して基底を求める)
...
開発中の作業版には、右クリックから8方向の道具を選ぶ操作と、点から穴のある板を作って保存する初回案内を追加しています。入力中にF1で説明を開いて戻る操作、説明の検索、キー移動で入力欄の説明を読む操作も確認しています。現在は全体確認と公開準備を進めており、正式配布版としての完了前です。[右クリックで道具を選ぶ](packages/help-content/docs/ja/radial-menu.md) · [初めての作図](packages/help-content/docs/ja/tutorial.md)
