...
### 0.a 追記(2026-09-05、実装中の決定)

実装が進む中でタスク8・12・13・14・19・20 の担当から出た確認事項に、統括が下した決定を記す(一次情報は `docs/報告記録.md` の「## 2026-09-05 00:25 P4 完了(§5.5 の照合)、P4b タスク7 完了、コミットの待ち行列」の項とその追記)。**既存の §0 〜 §0.a の記述は消さず、本節は追記・注記として扱う。**

1. **`SketchErrorCode` に `constraintUnsolved` を追加する**(`docs/報告記録.md` 2026-09-05 実時計 01:15 追記)。`constraintConflict` は原因の拘束を指し示せるときに使い、指し示せない場合は `constraintUnsolved` とする(3D スケッチへ拘束を付けようとしたとき・原因を絞れない未収束・式で表せない拘束の 3 つ)。タスク4 の `SketchErrorCode` に足す表(旧 2 種)は 3 種に改め、タスク8 の検証表の該当行にも注記を足した(本ファイル該当箇所)。理由: 拘束を特定できない失敗を無理に `constraintConflict` に押し込めると、一覧で赤く光らせる拘束を誤って示すため。影響: タスク4・タスク8・タスク12(断りの文言)・タスク13(印の色・一覧)。
2. **`diagnosis` は `ConstraintDiagnosis | null`**(拘束 0 個は診断しない、2026-09-05 実時計 01:15 追記、タスク8 で実装済み)。`radiusOverrides` の鍵は `radiusComponentKey` とする。拘束を解いた座標は文書に書かず、`pointOverrides` / `radiusOverrides` として `resolveSketch` へ渡し、差し込みは `resolveSketch` の中で行う(`resolveCoordinate` は鍵の規約を 2 か所に持たせないため触らない)。理由: 「導出できるものは保存しない」(`rules/04`)と、鍵の規約の一元化。影響: タスク8(実装済み)・タスク12・13(診断の参照先)。
3. **タスク14 のドラッグは (a) 軟らかい目標+拘束優先**(2026-09-05 実時計 01:15 追記)。引っぱりの残差 2 行に小さい重み(初期値 1e-3)を掛け、拘束を先に満たしたうえで残る自由度で目標へ寄せる。硬い一時固定は水平拘束などを破るため不採用。重みは手触りで調整する。理由: 拘束が優先されないと「引っぱると条件が保たれない」ことになり FR-313 の完了条件と矛盾するため。影響: タスク14 の `solveWithDrag` と検証表・落とし穴(本ファイル該当箇所に反映済み)。
4. **矛盾する拘束は事前に判定せず、付けたうえで赤い印+帯の理由で示す**(止めない、`rules/04`。§0.a に決定が無かったため rules/04 の原則を優先してタスク12 で実装済み、2026-09-05 実時計 01:50 追記)。同じ拘束の照合(冗長・重複の判定)は全種類で選択順序を無視し、順序が意味を持つのは角度拘束だけ(符号つき、1 本目→2 本目の順)。水平・垂直は線分 1 本の選択だけを認める(`SketchDocument` の型の都合)。影響: タスク4(照合ロジック)・タスク12(1751 行の落とし穴、本ファイルに反映済み)・タスク7(診断)。
5. **端点(線分の端など)の当たり判定**(2026-09-05 実時計 01:50 追記)。拘束の道具が活性のときだけ端点の当たり判定(12 画素)を足し、`vertex` の指し先で `constraintReadiness` / `commitConstraint` へ渡す(タスク13)。通常の選択(拘束の道具が非活性)では従来どおり要素単位のまま。理由: 一致・対称など点を指定する拘束は、画面から端点を選べないと付けられないため。影響: タスク13(本ファイルに反映済み)・タスク12。
6. **タイムラインは §0.a-0.18 案 B の実装形で確定**(2026-09-05 実時計 01:05・01:35 追記)。モデルブラウザ(フィーチャーツリー)の行の左端のレール+つまみに実装し(独立区画は作らない)、`timelineIndex` は文書に保存しない(ストアの表示専用の状態)。3D へ出す形だけ `documentUpTo` で切る。**戻したまま作ると、つまみの位置へ差し込む**(タスク20。タスク19 の暫定「末尾へ戻して知らせる」は置き換え済み。タスク19 の下書きにあった「戻したままの作成を禁じる」は不採用で、本ファイルの当該箇所へ注記済み)。並べ替えは pointer イベントの自前実装、並べ替え後のつまみは通し番号のまま据え置き、自動スクロールは入れない。つまみの見せ方(初回の案内 / つまみを太くする / 下に帯を出す)は利用者に確認中で、無回答のときは初回の案内を既定とする。影響: タスク19・タスク20(実装済み)・タスク22(つまみの見せ方の確認待ち)。
7. **帯の 1 文の優先順位**(タスク16 向け): 失敗 > 中止 > 進み具合 > 知らせ > 計算中 > 向きの吸着 > 点の吸着 > 道具の案内。コマンドラインの断りは「基準ジオメトリの断り」の直後に置く(タスク18)。タイムラインの札(「途中まで戻しています」)は独立の札として扱う(タスク19、他の帯の 1 文と競合しない)。影響: タスク16・18・19 の `statusText.ts` の優先順位実装。
8. **パラメータ表は右区画(280px)に合わせた縦積み**とする(横 5 列ではない。タスク11 で実装済み、2026-09-05 実時計 01:40 追記)。確定は Enter / blur のみ、変数表はプロパティの欄・その場入力・コマンドラインの 3 入口へ同じ表を配線する(タスク11)。書き手は `constraints` を「あるときだけ書く」形にする(往復は deep equal で固定、タスク21)。影響: タスク11(実装済み)・タスク21。
9. **タスク22への持ち越し(一覧、2026-09-05 の各追記より)**:
   - ロールバック中、後ろの段のプロパティの言い回しを「計算できていません」→「まだ作られていません」に直す(戻り中は失敗ではないため)。
   - `consumedIds` は文書全体から導くため、途中まで戻しても「統合済み」の札が残る(表示上の既知差)。
   - 交点の候補は 12 画素以内のときだけ採り、外れたら近い方の 1 本に落とす(仕上げ (a) の実測: ほぼ平行な 2 本の交点は最大 51.57px 離れることがある)。
   - 削除の断りに参照元のフィーチャー名を出すため、model に「式と所有者 id の組」を返す口を新設する(現状は件数のみ)。
   - ばねの導出式(`solidCommands.ts:302` / `solidSummary.ts:916` の `evaluatedExpressionValue`)に変数表を渡す(現状はパラメータ名を書くと読み取り専用欄が `= 0` になる)。
   - `ROW_MENU_HEIGHT` は 128 → 192 に確定済み(タスク20)。
   - 左区画・右区画が写るヘルプのスクリーンショットは古いので撮り直す(撮影は統括の作業)。
...
packages/model/src/sketch/types.ts                   変更: SketchDocument.constraints、SketchErrorCode に 3 種(§0.a 追記 1)
...
**先行タスク:** なし
**対応要件:** FR-313
**関係する過去の失敗:** `docs/報告記録.md` 2026-09-04 11:05(P4 タスク4。`SketchFeatureKind` に種類を足したら、網羅型 `Record` を持つ 3 か所(model の `createSketchDocument.ts`、ui の `FeatureTree.tsx` / `featureSummary.ts`)が型検査で落ち、他の担当のコミットを止めた)。**本タスクは `SketchErrorCode` に 3 種(§0.a 追記 1)を足すので同じことが起きうる。** 手順1 で網羅箇所を Grep する。
...
- 変更: `packages/model/src/sketch/types.ts`(`SketchDocument.constraints`、`SketchErrorCode` に 3 種。§0.a 追記 1)
...
`SketchErrorCode` に足す 3 種(**注記(2026-09-05、§0.a 追記 1): 当初案は 2 種だったが、`constraintConflict` は原因の拘束を指せるときだけに使い、指せない場合に `constraintUnsolved` を新設して 3 種とする**):

| コード | いつ出るか | 文言 |
|---|---|---|
| `constraintConflict` | 反復が尽きても残差が残る、または冗長な拘束があり、**原因の拘束を指し示せる** | 「この拘束は同時には成り立ちません: ○○1、○○2」 |
| `constraintUnsolved` | **原因の拘束を指し示せない**(3D スケッチへ拘束を付けようとした・原因を絞れない未収束・式で表せない拘束) | 拘束を特定しない一般の断り文言(タスク12・13 で確定) |
...
- [ ] 4. `SketchDocument.constraints` と `SketchErrorCode` の 3 種(§0.a 追記 1)を足す。
- [ ] 5. 検査を書く。
- [ ] 6. `model/src/index.ts` へ輸出を足す。
...
| 矛盾する拘束(原因の拘束を指せる場合) | `errors` に `constraintConflict` 1 件、`resolved` は最後の x の形、例外なし | FR-504、NFR-RE-1 |
...
| 3D スケッチに拘束を足した文書 | `errors` に `constraintUnsolved` 1 件(原因の拘束を指せない 3 例の 1 つ)、形は拘束を無視 | §0.3、§0.a 追記 1 |
...
- **選択の順序に意味を持たせない拘束と、持たせる拘束を分ける。** 接線(線分+円)と対称(点 2 つ+軸)は**種類で見分ける**ので順序は不問。角度は「1 本目から 2 本目へ測る」ので順序が意味を持つ。**どちらかを報告に明記する。**(**決定(§0.a 追記 4、2026-09-05):** 同じ拘束の照合〔`ConstraintSummary` の重複・冗長判定を含む〕は全種類で選択順序を無視し、順序が意味を持つのは角度拘束だけ〔符号つき〕。水平・垂直は線分 1 本の選択だけを認める〔`SketchDocument` の型の都合〕。矛盾する拘束は事前に判定せず、付けたうえで赤い印+帯の理由で示す〔止めない、`rules/04`〕。)
...
- **端点の当たり判定(決定、§0.a 追記 5、2026-09-05):** 線分の端などの端点は、拘束の道具が活性のときだけ当たり判定(12 画素)を足し、`vertex` の指し先(`SketchElementRef` の相当の形)で `constraintReadiness` / `commitConstraint` へ渡す。拘束の道具が非活性のとき(通常の選択)は既存どおり要素単位のままで、端点は選べない。
...
/** 引っぱっている間の解。**決定(§0.a 追記 3、2026-09-05): 硬い一時固定ではなく、軟らかい目標+拘束優先**で解く
 *  (引っぱりの残差 2 行に小さい重み〔初期値 1e-3〕を掛けて残差へ加え、拘束を先に満たしたうえで
 *  残る自由度で目標へ寄せる。文書は変えない)。 */
...
**落とし穴:**
- **「長さ 10 が固定された線分の終点を引っぱるとどちらが動くか」は最小移動の解が決める。** 直感と合わないことがあるので、**担当が実測して期待値を固定し、その挙動を報告に書く**(推測で期待値を書かない)。**決定済み(§0.a 追記 3、2026-09-05): 「引っぱっている点の重みを上げる」方式を採用**(硬い一時固定は水平拘束などを破るため不採用。引っぱりの残差に小さい重み〔初期値 1e-3〕を掛け、拘束を先に満たしたうえで残る自由度で目標へ寄せる。重みは手触りで調整する)。
...
- **つまみを戻したまま新しいフィーチャーを作ると、末尾ではなく途中へ入る**(タスク20 の範囲)。本タスクの下書きでは「戻したままの作成を禁じる」としていたが、**統括の承認(2026-09-05 実時計 01:05)により「禁じる」ではなく末尾へ戻して知らせる**(rules/04 の「止めない」)を暫定実装とし、**タスク20 で `insertPositionAt` によりつまみの位置へ差し込む形に置き換え済み**(§0.a 追記 6)。「止めない」の原則により、いずれの段階でも作成そのものを止めたことはない。
