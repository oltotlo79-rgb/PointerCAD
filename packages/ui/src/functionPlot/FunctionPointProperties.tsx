...
export function FunctionPointProperties({pointId,reference}:{readonly pointId:string;readonly reference:FunctionPointReference}):React.JSX.Element {
  ...
  return <section className="pcad-section" data-help-topic="function-point">
    ...
    <button title={t('controlGuide.button.pointEdit')} type="button" onClick={()=>setOpen(true)}>{t('functionPoint.edit')}</button>
    {reference.direction===undefined?<button title={t('controlGuide.button.functionDirection')} type="button" onClick={()=>setDirection(true)}>{t('functionDirection.title')}</button>:null}
