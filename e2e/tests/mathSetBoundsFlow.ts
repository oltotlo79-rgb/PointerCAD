...
export async function mathSetBoundsFlow(page: Page, info: TestInfo, app?: ElectronApplication): Promise<void> {
  ...
  await name.fill('集合の上限'); await name.press('Enter');
  ...
  for (const [source, message] of [
    ['setmax(interval(0,open(1)))', '最大値または最小値が存在しません'],
    ['sup(∅)', '空集合です'],
    ['0*setmax(interval(0,open(1)))', '最大値または最小値が存在しません'],
  ]) {
  ...
  const source = 'sup(interval(open(0),open(4)))', changed = 'sup(interval(open(0),open(6)))';
  await input.fill(source); await expectResult(result).toHaveText('= 4');
  await page.setViewportSize({ width: 1440, height: 1100 });
  await captureManualDetail(page, info, { name: 'math-set-bounds', dialog, script: new URL(import.meta.url),
    fixture: { source, expected: 4, maximumExists: false, angleUnit: 'degree' } });
  await page.setViewportSize({ width: 1440, height: 900 });
  ...
  const popover = page.locator('.pcad-popover'), coordinate = 'coef("集合の上限")';
  ...
  const saved = await savePart(page, info, 'set-bounds.pcad', app);
  expect(saved.parameters.find(parameter => parameter.name === '集合の上限')?.value).toMatchObject({ source, mathDefinition: { source }, value: 4 });
  ...
  await reopenPart(page, info, 'set-bounds.pcad', app);
  await page.getByRole('tab', { name: 'パラメータ', exact: true }).click();
  await row.getByRole('button', { name: '数式で入力', exact: true }).click();
  await waitForMathEditorText(dialog); await expect(input).toHaveValue(source); await expectResult(result).toHaveText('= 4');
  await page.keyboard.press('F1'); await expect(page.locator('.pcad-help__article')).toContainText('集合の上限・下限と最大値・最小値');
  ...
  const edited = await savePart(page, info, 'set-bounds-edited.pcad', app);
  ...
  expect(edited.parameters.find(parameter => parameter.name === '集合の上限')?.value.source).toBe(changed);
  await undoMathEdit(page);
  const undone = await savePart(page, info, 'set-bounds-undone.pcad', app);
  ...
  expect(undone.parameters.find(parameter => parameter.name === '集合の上限')?.value.source).toBe(source);
}
