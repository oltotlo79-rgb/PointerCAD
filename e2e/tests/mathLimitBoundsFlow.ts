...
export async function mathLimitBoundsFlow(page: Page, info: TestInfo, app?: ElectronApplication): Promise<void> {
  ...
  await name.fill('上極限の値'); await name.press('Enter');
  ...
  await input.fill('limsup(1/t,t,0)');
  await expectResult(result.locator(':scope > p').first()).toHaveText('= +∞');
  await expect(result).toContainText('無限大は座標には使えません'); await expect(apply).toBeDisabled();
  for (const [source, message] of [
    ['limsup(n,n,0,0,ℤ)', '式の成立条件または計算結果を確認してください'],
    ['0*limsup(t,t,∞)', '式の成立条件または計算結果を確認してください'],
  ]) {
    await input.fill(source); await expectResult(result).toContainText(message);
    await expect(result).toHaveClass(/pcad-math-editor__result--error/u); await expect(apply).toBeDisabled();
  }
  const source = 'limsup(4*(-1)^n,n,∞,0,ℤ)', changed = 'limsup(6*(-1)^n,n,∞,0,ℤ)';
  await input.fill(source); await expectResult(result).toHaveText('= 4');
  await page.setViewportSize({ width: 1440, height: 1100 });
  await captureManualDetail(page, info, { name: 'math-limit-bounds', dialog, script: new URL(import.meta.url),
    fixture: { source, expected: 4, approachDomain: 'integers', angleUnit: 'degree' } });
  await page.setViewportSize({ width: 1440, height: 900 });
  ...
  await expectResult(result).toHaveText('= 4');
  await expect(input).toHaveValue(/ℤ/u);
  await input.fill(source); await expectResult(result).toHaveText('= 4');
  ...
  const popover = page.locator('.pcad-popover'), coordinate = 'coef("上極限の値")';
  ...
  const saved = await savePart(page, info, 'limit-bounds.pcad', app);
  expect(saved.parameters.find(parameter => parameter.name === '上極限の値')?.value).toMatchObject({ source, mathDefinition: { source }, value: 4 });
  ...
  await reopenPart(page, info, 'limit-bounds.pcad', app);
  await page.getByRole('tab', { name: 'パラメータ', exact: true }).click();
  await row.getByRole('button', { name: '数式で入力', exact: true }).click();
  await waitForMathEditorText(dialog); await expect(input).toHaveValue(source); await expectResult(result).toHaveText('= 4');
  await page.keyboard.press('F1'); await expect(page.locator('.pcad-help__article')).toContainText('上極限・下極限');
  ...
  const edited = await savePart(page, info, 'limit-bounds-edited.pcad', app);
  ...
  expect(edited.parameters.find(parameter => parameter.name === '上極限の値')?.value.source).toBe(changed);
  await undoMathEdit(page);
  const undone = await savePart(page, info, 'limit-bounds-undone.pcad', app);
  ...
  expect(undone.parameters.find(parameter => parameter.name === '上極限の値')?.value.source).toBe(source);
}
