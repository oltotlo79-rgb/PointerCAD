...
部品の図面で「穴表」を選び、「穴の座標を測る図」と「穴座標の基準点」を指定して「表を配置」を押します。基準点のX・Y・Zは元の部品座標です。表を用紙へ置く位置のX・Yとは別です。

穴とねじ穴から、記号・X・Y・直径・深さをまとめます。X・Yは選んだ図の向きで測った実寸で、縮尺を変えても値は変わりません。同じ径をA1・A2、次の径をB1のように分類し、図中にも同じ記号と穴中心への引出線を付けます。貫通穴は「貫通」と表示します。
