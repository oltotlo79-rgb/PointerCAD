# 指示書 P7 タスク11a-2(第 3 回。recomputePart の onResolved を本担当の所有で足し、UI 本配線を完了する): アセンブリの文書の実経路(ui 側): attachAssembly・保存 / 開く・Undo・dirty・自動保存・ビューポートの実配線(レビュー R4-1・R6-1。工数 L)
