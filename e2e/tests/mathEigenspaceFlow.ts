...
/** Select an eigenvector component explicitly and retain the complex-part choice through history. */
export async function mathEigenspaceFlow(page: Page, info: TestInfo, app?: ElectronApplication): Promise<void> {
  ...
  await name.fill('固有方向の成分'); await name.press('Enter');
  ...
  await input.fill('eigenspace([[0,-1],[1,0]],i)');
  ...
  const selected = 'tensorelement(eigenspace([[0,-1],[1,0]],i),[1,1])';
  await expect(input).toHaveValue(selected); await expect(apply).toBeDisabled();
  const source = `Im(${selected})`;
  await input.fill(source); await expect(dialog.locator('[role="status"]')).toHaveText('= 1');
  await page.keyboard.press('F1');
  await expect(page.locator('.pcad-help__article')).toContainText('固有値を指定して基底を求める');
  await page.keyboard.press('Escape');
  await page.screenshot({ path: info.outputPath('math-eigenspace-complex-component.png'), fullPage: true });
  await captureManualDetail(page, info, {
    name: 'math-eigenspace-complex-component', dialog, script: new URL(import.meta.url),
    fixture: { kind: 'mathematics-input-example', parameter: '固有方向の成分',
  ...
  const saved = await savePart(page, info, 'math-eigenspace.pcad', app);
  expect(saved.parameters.find(parameter => parameter.name === '固有方向の成分')?.value).toMatchObject({ value: 1, source });
  await reopenPart(page, info, 'math-eigenspace.pcad', app);
  await page.getByRole('tab', { name: 'パラメータ', exact: true }).click();
  await row.getByRole('button', { name: '数式で入力', exact: true }).click();
  await expect(input).toHaveValue(source);
  const edited = 'Im(tensorelement(eigenspace([[0,-1],[1,0]],-i),[1,1]))';
  await input.fill(edited); await expect(dialog.locator('[role="status"]')).toHaveText('= -1');
  await apply.click(); await expect(dialog).toHaveCount(0);
