...
export function SheetUnfoldPanel({ session }: { readonly session: SheetMetalToolSession }): React.JSX.Element {
  ...
  return <form className="pcad-section pcad-sheet-metal" aria-label={t('sheetMetal.unfold')} data-help-topic="sheet-metal-flat" onSubmit={(event) => {
    ...
    {targets.length === 0 ? <p>{t(computing ? 'sheetMetal.waitingForBody' : 'sheetMetal.needBase')}</p> : <>
      <label className="pcad-field"><span>{t('sheetMetal.target')}</span><select title={t('sheetMetal.guide.target')} value={target?.id ?? ''} onChange={(event) => {
      ...
      <label className="pcad-field"><span>{t('sheetMetal.fixedPanel')}</span><select title={t('sheetMetal.guide.fixedPanel')} value={fixed} onChange={(event) => { clear(); setFixedId(event.target.value); }}>
      ...
      <fieldset><legend>{t('sheetMetal.seams')}</legend><p>{t('sheetMetal.seamsHint')}</p>
        {sheet?.bends.map((bend, index) => <label className="pcad-sheet-metal__check" key={bend.id}>
          <input title={t('sheetMetal.seamsHint')} type="checkbox" checked={selectedSeams.includes(bend.id)} onChange={(event) => {
    ...
    <div className="pcad-sheet-metal__actions"><button className="pcad-button pcad-button--primary" type="submit" title={t('sheetMetal.guide.showFlat')} disabled={target === undefined || computing || requested}>{t('sheetMetal.showFlat')}</button>
      <button className="pcad-button" type="button" title={t('sheetMetal.guide.showFolded')} onClick={clear}>{t('sheetMetal.showFolded')}</button></div>
    ...
    <fieldset><legend>{t('sheetMetal.output')}</legend><p>{t('sheetMetal.outputHint')}</p>
      <div className="pcad-sheet-metal__actions">{(['flatDxf', 'flatStep', 'foldedStep'] as const).map((format) =>
        <button className="pcad-button" key={format} type="button" title={t(`sheetMetal.guide.${format}`)} disabled={preview?.flat === undefined || computing || requested}
          onClick={() => { if (preview !== null) void exportSheet(preview, format); }}>{t(`sheetMetal.${format}`)}</button>)}</div>
    </fieldset>
    <button className="pcad-button" type="button" title={t('sheetMetal.guide.cancel')} onClick={() => useAppStore.getState().closeSheetMetalTool()}>{t('sheetMetal.close')}</button>
  </form>;
}
