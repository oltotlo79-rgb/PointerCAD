  ...
  switch (tool) {
    ...
    */
    ...
    /*
      ...
      |  | | ただし**切り替えはしない**(`keepsSelectionKind` が真。理由はそちらの注釈) |
      | エンボス | 面 | 彫る相手の平らな面を選ぶ |
      | 外ねじ | 面 | ねじを切る円柱の面を選ぶ |
 ...
 * 「選んでから道具」の順で成立しなくなる。
 *
 * **くり抜き**(FR-418、タスク50・52)もこれに当たる(タスク52 の申し送り、2026-09-06 実測)。
 * くり抜きが**必ず要るのは対象の立体 1 つ**で、開ける面は 0 枚でもよい(`commitShell` の
 * `openFaces` は空でよく、全面くり抜き = 開口なしの中空になる)。ところが
 * `selectionKindForTool('shell')` は「選ぶなら面」の意味で `face` を返すので、
 * **立体を選んでから「くり抜き」を押すと `kindChanged` で選択が空になり**、
 * 押した直後に「対象になる立体が選ばれていません」になって全面くり抜きが作れなかった。
 * 切り替えないことで、①立体を選んでから押す(全面くり抜き)と②面を選んでから押す
 * (`shapeEditTargetOf` が面の属する立体を対象にする)の**どちらの順でも成立する**
 * (NFR-UX-1)。開ける面を後から足したいときは `3` キーで「面」へ切り替えて選ぶ。
 *
 * **抜き勾配・エンボス・外ねじはここに入れない**(2026-09-06 に同じ経路で確かめた)。
 * この 3 つは**面が必須**(抜き勾配は基準の平らな面+傾ける面、エンボスは平らな面、
 * 外ねじは円柱の面)で、立体だけを選んだ状態からは道具を押しても作れない。切り替えれば
 * その場でビューポートの面を押せる状態になり、必要な面を**後から足せる**ので、
 * 切り替えるほうが利用者の意図に近い(面を先に選んでから押した場合は種類が変わらないので
 * 選択はそのまま残る)。くり抜きだけが「立体だけで完成する」点で違う。
 *
 * **球面上の点**(FR-431、タスク22)もこれに当たる。対象は「どの球か」の 1 つだけで、
 * それは**立体そのものを選んでいても球面(面)・稜線・頂点を選んでいても同じ球に行き着く**
 * (`sketchCommands.ts` の `selectedSphereFeature` が `立体の id#face:0` の区切りより前で
 * 引くため。§0.a-0.21 の「球のボディ(または球面)を選んだとき」そのもの)。ここで `body` へ
 * 決め打ちで切り替えると、球面を押して選んでから道具を押した瞬間にその選択が消え、
