...
export function createMathPartRecomputer(createClient: () => MathWorkerClient) {
  ...
  return async (document: PartDocument, bridge: KernelBridge, options: PartRecomputeOptions = {}): Promise<PartRecomputeResult> => {
    ...
    const curves = (hasSurfaces || document.sketches.some(sketch => sketch.features.some(feature => feature.kind === 'functionCurve')))
      && options.functions === undefined ? createBrowserFunctionClient() : undefined;
    const surfaces = hasSurfaces && options.functions === undefined ? createBrowserFunctionSurfaceClient() : undefined;
    ...
    const implicitSurfaces = hasImplicitSurfaces && options.functions === undefined ? createBrowserImplicitSurfaceClient() : undefined;
    ...
    const implicitCurves=hasImplicitCurves && options.functions===undefined ? createBrowserImplicitCurveClient() : undefined;
    const points=curves===undefined?undefined:createBrowserFunctionPointContinuationClient();
