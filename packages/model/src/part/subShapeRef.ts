 ...
 *
 * 型の置き場について: `SubShapeRef` / `SubShapeFingerprint` / `SubShapeKind` /
 * `FaceSurfaceKind` / `EdgeCurveKind` は `.pcad` に保存される値なので、保存形をまとめる
 * `part/types.ts` に置く(`docs/報告記録.md` 2026-09-03 07:35 の④「解決用の型は保存形の
 * types.ts ではなく使う側のファイルへ置く」の裏返しで、保存される型は types.ts へ集める)。
 * タスク12 ではこのファイルへ暫定で置いていたが、計画書の段取りどおりタスク13 で
 * `part/types.ts` へ移し、ここは import に差し替えた(型の定義を2か所に置かない)。
 *
...
import { keyNumber } from './cacheKey.js';
import type { SubShapeKind, SubShapeRef } from './types.js';
