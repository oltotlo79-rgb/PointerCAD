...
export function DrawingDimensionSeriesPanel(): React.JSX.Element | null {
  ...
  return <form className="pcad-drawing-settings" aria-label={t('drawing.series.title')} data-help-topic="dimension-series"
    ...
    <label title={t('drawing.series.kind.controlHint')}>{t('drawing.series.kind')}<select aria-label={t('drawing.series.kind')} value={editor.seriesKind} onChange={(event) => {
    ...
    {editor.seriesKind === 'coordinate' ? null : <label title={t('drawing.series.axis.controlHint')}>{t('drawing.series.axis')}<select aria-label={t('drawing.series.axis')} value={editor.axis}
    ...
    {editor.seriesKind === 'chain' ? null : <label title={t('drawing.series.base.controlHint')}>{t('drawing.series.base')}<select aria-label={t('drawing.series.base')} value={editor.baseIndex}
    ...
    <label title={t('drawing.series.offset.controlHint')}>{t('drawing.series.offset')}<input value={editor.offset} inputMode="decimal" onChange={(event) => update({ offset: event.target.value })} /></label>
    <div className="pcad-segmented"><button title={t('drawing.controlHint.applyDimensionPlacement')} type="submit" className="pcad-button" disabled={busy || targets.length < 2}>{t('drawing.action.apply')}</button>
      <button title={t('drawing.controlHint.closeDimensionPlacement')} type="button" className="pcad-button" onClick={() => useAppStore.getState().setDrawingTool('select')}>{t('drawing.action.close')}</button></div>
  </form>;
}
