...
describe('3D スケッチで押した場所に点を置く面(タスク14)', () => {
  /*
   * 斜め上から見下ろしたときの視線の一例。`freeClickPlane` は「カメラを向く面」を作るだけで
   * 視線の値そのものには依らないので、ホーム視点の実際の向き(`HOME_ORBIT` は 3 成分の
   * 大きさが等しい 1/√3)とは別のベクトルでよい(2026-09-06 に既定の視点を
   * 前・上・右へ変えたが、この検査の結果は変わらない)。
   */
