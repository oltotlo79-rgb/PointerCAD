 ...
 *
 * まったく同じ関数が `sketch/constraints/solve.test.ts` と
 * `sketch/constraints/diagnose.test.ts` の 2 ファイルへ写されていた
 * (`packages/kernel` 側で§0.a-0.88「全段をこの窓口に揃える」と決めたのを受け、
 * model 側でも揃える)。判定の決め(環境変数の名前・`'1'` という値・参考モードの文言)が
 * 複数箇所に散っていると、`scripts/check.ps1` 側の設定を変えたときに一部だけ古いまま
 * 残る危険がある。だから決めは 1 か所だけに置く。
 *
 * `packages/kernel` に同じ中身の関数があるが、依存方向は `model → kernel` の
 * 一方向であり、検査専用ヘルパーのためだけに import すると kernel 側の変更が
 * model の検査を無関係に揺らす結合を生むため、ここでは同じ中身をもう 1 つ置く。
 * **P6 の第 1 組の終わりに `@pointercad/test-utils` へ寄せる予定(利用者の承認待ち)。**
 *
 * このファイルは**検査からしか呼ばれない**ので、`src/index.ts` の輸出には入れない
 * (`packages/model/tsconfig.json` の `include` は `src` なので型検査には入る)。
 */
